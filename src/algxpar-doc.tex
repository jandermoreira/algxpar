...
\begin{figure}
    \tcbinputlisting{listing file = lzw.tex, listing only}
    \vspace{3em}
    \tcbinputlisting{listing file = lzw.tex, text only}
\end{figure}
...
\begin{macro}{AlgLanguageSet}[\MArg{language name}\MArg{keyword assignments}]
    ...
    To create a new set, copy the file \texttt{english.kw.tex} and edit it accordingly.
    ...
    In translations, these \textit{compound keywords} do not necessarily need to appear (see file \texttt{brazilian.kw.tex}, which follows the settings in \texttt{english.kw.tex}). However, if defined, there will be different versions for each language.
\end{macro}

\begin{figure}
    \tcbinputlisting{listing file = english.kw.tex}
\end{figure}
